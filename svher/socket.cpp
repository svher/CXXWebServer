...
namespace svher {
    ...
    int64_t Socket::getSendTimeout() {
        FdContext::ptr context = FdMgr::GetInstance()->get(m_sock);
        if (context) {
    ...
    bool Socket::bind(const Address::ptr addr) {
        if (!isValid()) {
            newSock();
            if (UNLIKELY_EXECUTED(!isValid())) {
                return false;
            }
        }
        if (UNLIKELY_EXECUTED(addr->getFamily() != m_family)) {
        ...
        if (::bind(m_sock, addr->getAddr(), addr->getAddrLen())) {
            LOG_ERROR(g_logger) << "bind errno=" << errno
    ...
    int Socket::sendTo(const iovec *buffers, int length, const Address::ptr to, int flags) {
        if (isConnected()) {
            msghdr msg;
            memset(&msg, 0, sizeof(msg));
            msg.msg_iov = (iovec*) buffers;
    ...
    bool Socket::cancelRead() {
        return IOManager::GetThis()->cancelEvent(m_sock, svher::IOManager::READ);
    }

    bool Socket::cancelWrite() {
        return IOManager::GetThis()->cancelEvent(m_sock, svher::IOManager::WRITE);
    }
